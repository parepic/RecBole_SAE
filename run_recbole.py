...
def create_visualizations_neurons():
    ...
    trainer = get_trainer(config["MODEL_TYPE"], config["model"])(config, model)
    arps = [0.00041477011735706386]
    ndcgs = [0.1212]
    hits = [0.2412]
    coverages = [0.6569086651053864]
    lt_coverages = [0.6451710566151984]
    deep_lt_coverages = [0.48591864103710325]
    dampen_percs = [0.0]  # still no value provided
    ginis = [0.7572878507064535]
    ndcg_heads = [0.1848]
    ndcg_mids = [0.1234]
    ndcg_tails = [0.0621]
    neuron_count = 0
    ...
    for change in tochange:
        if count==0:
            ...
            test_result = trainer.evaluate(
                valid_data, model_file=args.path, show_progress=config["show_progress"], N=4096, beta=change, gamma=100
            )
        count += 1
