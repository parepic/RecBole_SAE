...
def create_visualizations_neurons():
    ...
    count = 0
    # tochange = np.linspace(0, 2000, 3).tolist()
    ...
    for change in tochange:
        if count==0:
            ...
            test_result = trainer.evaluate(
                test_data, model_file=args.path, show_progress=config["show_progress"], N=2048, beta=change, gamma=0
            )
        count += 1
        ndcgs.append(test_result['ndcg@10'])
        ndcg_heads.append(test_result['ndcg-head@10'])
...
if __name__ == "__main__":
    ...
    if(args.model == "SASRec" and args.train):
        ...
        if(args.test):
            ...
            create_visualizations_neurons()
            # test_result = trainer.evaluate(
            #     test_data, model_file=args.path, show_progress=config["show_progress"], N=90, beta=-3, gamma=0
            # )
