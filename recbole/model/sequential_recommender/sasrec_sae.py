...
class SASRec_SAE(SASRec):
    ...
    def calculate_loss(self, interaction, scores=None, show_res=False):
        ...
        if self.mode == 'train':
            sae_loss = self.sae_module.fvu + self.sae_module.auxk_loss / 8
            if show_res:
                print(f"FVU: {self.sae_module.fvu}, AUXK Loss: {self.sae_module.auxk_loss}, AUXK Loss /8: {self.sae_module.auxk_loss / 32} SAE Total Loss: {sae_loss}")
        else:
